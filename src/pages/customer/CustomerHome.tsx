...
import {
  ...
  Sparkles, TrendingUp, ArrowRight, Loader2, Map, Store, Zap, ArrowUpDown, Navigation
} from "lucide-react";
...
const CustomerHome: React.FC = () => {
  ...
  const [realShops, setRealShops] = useState<ShopType[]>([]);
    ...
    <MobileShell>
      <div className="pb-28">
        ...
        {!userLocation && !showLocationModal && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mx-5 mt-5 p-4 glass rounded-3xl flex items-center gap-3 border-2 border-primary/20 shadow-sm relative overflow-hidden">
            <div className="absolute -right-4 -bottom-4 w-16 h-16 bg-primary/10 rounded-full blur-xl" />
            <div className="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center shrink-0">
              <Map size={18} className="text-primary" />
            </div>
            <div className="flex-1 z-10">
              <p className="text-xs font-extrabold text-foreground">Find nearby fast delivery</p>
              <p className="text-[10px] text-muted-foreground mt-0.5">Enable location for accurate shops</p>
            </div>
            <button onClick={fetchLiveLocation} disabled={isLocating} className="px-4 py-2.5 gradient-primary text-primary-foreground rounded-xl text-xs font-bold shadow-glow z-10 flex items-center gap-1 active:scale-95 transition-transform">
              {isLocating ? <Loader2 size={12} className="animate-spin" /> : "Allow GPS"}
            </button>
          </motion.div>
        )}
